...
import {
  json, badRequest, notFound, forbidden, clean, cleanDate, badDate, toCents, oneOf, readJson, now,
} from './util.js';
...
import { COMPANY_LEAD, NO_COMMISSION, splitPct, shareOf, UNSPLIT_COMMISSION_KINDS } from './split.js';
...
export async function handleBookingRecord(request, env, id) {
  ...
  return json({
    ...
    // present and refused. Not while acting as somebody: moving a reservation
    // between two other people's books is not something to do from inside one
    // of them.
    advisors: isAdmin(user) && !user.acting_as ? await db.advisorOptions(env, user) : [],

...
async function loadTrip(env, code) {
  ...
  // nothing else: no commission, no split, no agreed percentages, no lead
  ...
  const booking = await env.DB.prepare(
    `SELECT b.id, b.user_id, b.client_name, b.supplier, b.product_name, b.destination,
            ...
            u.seller_of_travel, u.agency_id
  ...
  const [travellers, components, options, payments, documents, itinerary] = await Promise.all([
    ...
    env.DB.prepare(`SELECT id, label, detail, amount_cents, chosen, recommended,
                           image_url, inclusions, chosen_at, chosen_by, component_id
                      FROM quote_options
 ...
 * with, so it cannot be allowed to carry anything the share code does not
 * already entitle somebody to see: a trip name, and the agency's colour.
 *
 * 256 is the real size of the file, declared as 256. Claiming 192 and 512 for
 * one image is the ordinary thing to do here and it is a lie a phone can act
 * on, by upscaling a blurred icon onto a home screen somebody looks at.
 */
export async function renderTripManifest(request, env, code) {
  ...
  return new Response(JSON.stringify({
    ...
    theme_color: accent,
    icons: [{ src: '/logo-mark.png', sizes: '256x256', type: 'image/png', purpose: 'any' }],
  }, null, 2), {
    headers: {
      'content-type': 'application/manifest+json; charset=utf-8',
      // Short, because the trip name on the icon should follow a rename
      // without the client reinstalling anything.
...
export async function renderTripPage(request, env, code) {
  ...
  // Somebody opened it.
  //
  // The portal records that a quote was sent and then stopped, so "did you get
  // my email" was still a phone call. Counted here, at the point the page is
  // actually rendered.
  //
  // ?preview is the advisor's own link from the reservation screen and is not
  // counted. Without that the number would mostly be the advisor checking
  // their own work, which is the fastest way to make a metric worthless.
  //
  // Best effort on purpose: a client reading their trip must never see an
  // error because a counter would not increment.
 ...
 * install. Travefy sells that as a product; on a page the client already has a
 * link to, it is a manifest and four meta tags.
 *
 ...
 * superseded document would be believed. Offline can come once there is
 * somewhere honest to put "shown as of Tuesday".
 */
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  /* A column, so the button sits on the floor of every card whatever the text
     above it does. Three cards with different amounts to say were three
     buttons at three heights. */
  ...
  .option.rec{border-color:var(--teal);box-shadow:0 0 0 1px var(--teal)}
  .tick.rec{background:var(--teal)}
  ...
  /* auto, so the button falls to the bottom of the card whatever the text
     above it does. Three cards with different amounts to say put three
     buttons at three heights. */
  ...
  /* The gap above the button comes from whatever sits above it, so the button
     itself can keep margin-top:auto and still not touch the text on the
     tallest card in a row. Margins do not collapse in a flex column. */
